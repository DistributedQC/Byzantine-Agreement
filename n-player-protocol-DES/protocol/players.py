...
from protocol.config import NUM_PLAYERS, NUM_ROUNDS, COMMANDER_NAME, LIEUTENANT_NAMES, N, M, NUM_LIEUTENANTS
...
class Player(aqnsim.Node):
    def __init__(self, sim_context: aqnsim.SimulationContext, name: str):
        super().__init__(
            ...
            ports=["distributor"] + [COMMANDER_NAME] + LIEUTENANT_NAMES, # All-to-all classical communication; port with your own name isn't used
            name=name
        )
        self.data_collector.register_attribute(self.name + "_BV")
        self.bit_vector = []
    ...
    def measure_qubit(self):
        ...
        if len(self.bit_vector) == (NUM_PLAYERS - 1) * NUM_ROUNDS:
            ...
            self.data_collector.update_attribute(self.name + "_BV", self.bit_vector)
